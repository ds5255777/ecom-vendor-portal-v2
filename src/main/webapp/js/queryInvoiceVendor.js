...
function getData() {
	...
	$.ajax({
		type: "POST",
		data: "",
		url: "invoiceController/getAllQueryInvoiceVendor",
		dataType: "json",
		...
		success: function(data) {
			...
			if (data.msg == 'success') {
				...
				for (var i = 0; i < result.length; i++) {
					...
					var view = "<a href=\"#\" data-toggle=\"modal\" data-target=\"#tripValue\" class=\"invoiceViewByInvoiceNumber\" onclick=\"getInvoiceDataFormDataByInvoiceNumber('" + result[i].ecomInvoiceNumber + "','InvoiceQuery Queue')\" >" + result[i].ecomInvoiceNumber + "</button>";
...
$('#tabledata tbody').on('click', ".invoiceViewByInvoiceNumber", function() {
	...
	getInvoiceDataFormDataByInvoiceNumber((row.cells[0].innerText), "InvoiceQuery Queue");
});
