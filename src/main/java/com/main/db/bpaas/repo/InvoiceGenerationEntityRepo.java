...
public interface InvoiceGenerationEntityRepo extends JpaRepository<InvoiceGenerationEntity, Long> {
	...
	/*
	 * @Query(value =
	 * "select * from invoice_generation where invoice_status='Draft-Invoicing'  and vendor_code=?"
	 * , nativeQuery = true) List<InvoiceGenerationEntity>
	 * getAllRejectInvoice(String vendorCode);
	 */
	...
	@Query(value = "select count(*) from invoice_generation where invoice_status in('Approved','Pending For Approval','In-Review')", nativeQuery = true)
