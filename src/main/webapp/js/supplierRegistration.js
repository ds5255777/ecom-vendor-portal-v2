...
function defautValueShown() {
	debugger
	$("#businessClassification").val("Other Enterprise");
	$("#sectionType").val("Individual");
	$("#enInvApplicable").val("No");
	$("#tdsApplication").val("No");
	changetextbox();
}
// new by mayank
...
$("#addBookGridButt").click(function() {
	...
	// BY MAYANK
	//var abc5 = document.getElementById('addressType').value;
...
$("#contactDetailsButt").click(function() {
	...
	var abc5 = document.getElementById('supplierTypeInContact').value;
	var abc6 = document.getElementById('addressTypeInContact').value;
	if (abc == null || abc == "") {
		swal.fire("Alert", "First Name is mandatory !", "warning");
		return false;
	} else if (abc3 == null || abc3 == "") {
		...
		swal.fire("Alert", "Supplier Type is mandatory !", "warning");
		return false;
	}else if (abc6 == null || abc6 == "") {
		swal.fire("Alert", "Address Type is mandatory !", "warning");
		return false;
	}
	 else {
		 debugger
		...
		for (var i = 0; i < rows.length; i++) {
          ...
          if (cells[4].innerHTML === abc5 && cells[8].innerHTML === "Primary" ) {
            netCount++
    }
    }
    
      if(netCount < 1){
		swal.fire("Alert", "Please add data in address book against this Supllier Type and Address Type", "warning");
		return false;
	}
	}else{
		for (var i = 0; i < rows.length; i++) {
          ...
          if (cells[4].innerHTML === abc5) {
            netCount++
    }
    }
    
      if(netCount < 1){
		swal.fire("Alert", "Please add data in address book against this Supllier Type ", "warning");
		return false;
	}
	}
	
	 var table = document.getElementById('contactDetailsGrid');
		var rows = table.rows;
		var contactCount = 0 ;
		for (var i = 0; i < rows.length; i++) {
          var cells = rows[i].cells;
          if(abc6 === "Primary"){
          if (cells[4].innerHTML === abc5 && cells[5].innerHTML === "Primary" ) {
            contactCount++
    }
    }
    }
      if(contactCount >= 1){
		swal.fire("Alert", "Primary Type Address is already exist against the Supplier Type", "warning");
		return false;
		...
		$("#contactDetailsGrid").append('<tr class=""><td>' +
			...
			document.getElementById('conLname').value + '</td><td>' +
			document.getElementById('conPhone').value + '</td><td>' +
			document.getElementById('conEmail').value + '</td><td>' +
			document.getElementById('supplierTypeInContact').value + '</td><td>' +
			document.getElementById('addressTypeInContact').value + '</td><td>  <a href="#" class="removeContact btn btn-danger btn-sm" value="abc">Remove</a></td></tr>');

		document.getElementById('conFname').value = "";
		document.getElementById('conLname').value = "";
		document.getElementById('conPhone').value = "";
		document.getElementById('conEmail').value = "";
		document.getElementById('supplierTypeInContact').value = "";
		$("#supplierTypeInContact").val('').trigger('change');
		document.getElementById('addressTypeInContact').value = "";
	}
});
...
$("#addBankGridButt").click(function() {
	...
	if (abc == null || abc == "") {
		...
		url: "ajaxController/checkBankDetailIsExistOrNot",
		dataType: "json",
		...
		success: function(response) {
           ...
           bankValidation = "true" 
			} else if(response.msg == 'Alreday Exist') {
	...
	else if(bankValidation == "false"){
		swal.fire("Alert", "Bank is already assigned to other Vendor", "warning");
		return false;
		}
...
function sendToServer() {
    ...
    debugger
    var addressBookId = "addBookGrid"; // Replace "yourTableId" with the actual ID of your table
    var addressBookTable = document.getElementById(addressBookId);
    var addressBookRows = addressBookTable.querySelectorAll("tr");
    var addressBookRowCount = addressBookRows.length;
    
    
   // if(addressBookRowCount != contactDetailRowCount){
	//	swal.fire("Alert", "No. of rows in Address Book does not match with No. of rows in Contact Detail", "warning")
	//	return false;
	//}
...
$("#paymentMethod").bind("click", function() {
	debugger
	alert("hhh")
