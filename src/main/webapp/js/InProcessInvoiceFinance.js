var csrfToken = document.cookie.replace(/(?:(?:^|.*;\s*)XSRF-TOKEN\s*\=\s*([^;]*).*$)|^.*$/, '$1');
            ...
            function getData() {
                ...
                $.ajax({
                    type: "POST",
                    data: "",
                    url: "financeController/getAllInReviewInvoice",
                    dataType: "json",
           headers: { 'X-XSRF-TOKEN': csrfToken },
                    ...
                    success: function(data) {
                        ...
                        if (data.msg == 'success') {
                            ...
                            for (var i = 0; i < result.length; i++) {
                                ...
                                var view = "<a href=\"#\" data-toggle=\"modal\" data-target=\"#tripValue\" class=\"invprofin\"  >" + result[i].ecomInvoiceNumber + "</button>";
            ...
            function refereshList() {
            	getData();
			                ...
			                $('#tabledata tbody').on('click', ".invprofin", function() {
                                var row = $(this).parents('tr')[0];
                                //console.log(row.cells[0].innerText);
                                //console.log(tabledata.row(row).data().id);
                                getInvoiceDataFormDataByInvoiceNumber((row.cells[0].innerText),"In-Processed Invoice");
                });
