...
public interface TripDetailsRepo extends JpaRepository<TripDetails, Integer> {
	...
	@Query(value = "select  * from Trip_Details", nativeQuery = true)
	...
	@Query(value = "select * from trip_details where run_type='Ad-Hoc' and vendor_trip_status='Pending'", nativeQuery = true)
	...
	@Query(value="select count(*) from trip_details where run_type='Ad-Hoc' and vendor_trip_status='Approved'",nativeQuery=true)
