var csrfToken = document.cookie.replace(/(?:(?:^|.*;\s*)XSRF-TOKEN\s*\=\s*([^;]*).*$)|^.*$/, '$1');
            ...
            function getData() {
                ...
                $.ajax({
                    type: "POST",
                    data: "",
                    url: "financeController/viewAllProcessInvoiceForFinanceTeam",
                    dataType: "json",
                    ...
                    success: function(data) {
                        ...
                        if (data.msg == 'success') {
                            ...
                            for (var i = 0; i < result.length; i++) {
                                ...
                                var view = "<a href=\"#\" data-toggle=\"modal\" data-target=\"#tripValue\"  class=\"processinvnum\" >" + result[i].ecomInvoiceNumber + "</button>";
            ...
            function getInvoiceDataFormDataByInvoiceNumber(id, type) {
                ...
                $('.loader').hide();
                 ...
                 $('#tabledata tbody').on('click', ".processinvnum", function() {
                                var row = $(this).parents('tr')[0];
                                //console.log(row.cells[0].innerText);
                                //console.log(tabledata.row(row).data().id);
                                getInvoiceDataFormDataByInvoiceNumber((row.cells[0].innerText),"Processed Invoice");
                });
