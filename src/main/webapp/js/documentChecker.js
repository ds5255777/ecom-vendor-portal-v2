...
$(document).ready(function() {
	getVendorDetailsDocChecker();
	
});

function getVendorDetailsDocChecker() {

	$.ajax({
		type: "GET",
		url: "CommercialHeadController/findListOfAllVendorByDocumentChecker",
		dataType: "json",
			...
			if (data.msg == 'success') {
				...
				for (var i = 0; i < len; i++) {
					...
					tabledata2.row.add([
						...
						moment(result.data[i].createdOn).format(
							'DD-MM-YYYY')]);
